...
-- 작성자: SmartHR Team
-- 작성일: 2024-09-17
-- 설명: x_CreateCompany, x_UpdateCompany SP에 IsActive 파라미터 추가
-- =============================================
...
-- =============================================
-- 회사 등록 SP 업데이트 (IsActive 파라미터 추가)
-- =============================================
...
CREATE PROCEDURE x_CreateCompany
    ...
    @IsActive BIT = 1,              -- 상태 파라미터 추가 (기본값: 활성)
    @CreatedBy INT = NULL,
...
-- =============================================
-- 회사 수정 SP 업데이트 (IsActive 파라미터 추가)
-- =============================================
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- 회사 존재 여부 체크
        SELECT @Count = COUNT(*) FROM uCompanyTb WHERE CompanyId = @CompanyId;
        IF @Count = 0
        BEGIN
            SET @ResultCode = -1;
        ...
        UPDATE uCompanyTb SET
            ...
            IsActive = ISNULL(@IsActive, IsActive),      -- IsActive 필드 업데이트 추가
            UpdatedAt = GETDATE()
        WHERE CompanyId = @CompanyId;
...
PRINT '=== IsActive 파라미터가 추가된 Stored Procedures 업데이트 완료 ===';
-- =============================================
-- 작성자: SmartHR Team
-- 작성일: 2024-09-17
-- 설명: x_GetCompanies SP 간단 업데이트 (새로운 필드들만 추가)
-- =============================================

USE hr_system;
GO

-- 기존 SP 삭제
IF EXISTS (SELECT * FROM sys.objects WHERE name = 'x_GetCompanies')
    DROP PROCEDURE x_GetCompanies;
GO

-- 새로운 SP 생성
CREATE PROCEDURE x_GetCompanies
    @PageNumber INT = 1,
    @PageSize INT = 20,
    @IsActive BIT = NULL,
    @SearchKeyword NVARCHAR(100) = NULL,
    @Offset INT = 0,
    @ResultCode INT OUTPUT,
    @Message NVARCHAR(500) OUTPUT
AS
BEGIN
    SET NOCOUNT ON;

    BEGIN TRY
        -- 파라미터 검증
        IF @PageNumber < 1 SET @PageNumber = 1;
        IF @PageSize < 1 SET @PageSize = 20;
        IF @PageSize > 100 SET @PageSize = 100;

        SET @Offset = (@PageNumber - 1) * @PageSize;

        -- 회사 목록 조회 (새로운 필드들 포함)
        SELECT
            c.CompanyId,
            c.CompanyCode,
            c.CompanyName,
            c.CompanyNameEng,
            c.BusinessNumber,
            c.CorporateNumber,      -- 법인번호 추가
            c.CeoName,
            c.EstablishDate,
            c.PostalCode,           -- 우편번호 추가
            c.Address,
            c.AddressDetail,        -- 상세주소 추가
            c.PhoneNumber,
            c.FaxNumber,           -- 팩스번호 추가
            c.Email,
            c.Industry,
            c.BusinessType,        -- 업태 추가
            c.IsActive,
            c.CreatedAt,
            c.UpdatedAt,
            COUNT(*) OVER() AS TotalCount
        FROM uCompanyTb c
        WHERE
            (@IsActive IS NULL OR c.IsActive = @IsActive)
            AND (
                @SearchKeyword IS NULL
                OR c.CompanyName LIKE '%' + @SearchKeyword + '%'
                OR c.CompanyCode LIKE '%' + @SearchKeyword + '%'
                OR c.CeoName LIKE '%' + @SearchKeyword + '%'
                OR c.Industry LIKE '%' + @SearchKeyword + '%'
                OR c.BusinessType LIKE '%' + @SearchKeyword + '%'
            )
        ORDER BY c.CompanyId DESC
        OFFSET @Offset ROWS
        FETCH NEXT @PageSize ROWS ONLY;

        SET @ResultCode = 0;
        SET @Message = '회사 목록을 성공적으로 조회했습니다.';

    END TRY
    BEGIN CATCH
        SET @ResultCode = -1;
        SET @Message = '회사 목록 조회 중 오류가 발생했습니다: ' + ERROR_MESSAGE();
    END CATCH
END;
GO

PRINT '=== x_GetCompanies SP 업데이트 완료 ===';